...
$excelColumns = array('Case ID', 'Case Assignee First Name', 'Case Assignee Surname', 'Region', 'SSO', 'Receiving Country', 'Case Type', 'GE CoE Case Number', 'GE Business', 'Industry Focus Group', 'Case Initiation Date', 'Proposed Assignment From date', 'Work Ready Estimated Date (BM:1j)', 'First Contact with Applicant', 'All documents received from GE and Assignee to Prepare Application', 'Application sent to GE/ Assignee for Signature', 'All Documentation Received for Filing', 'Application Filed', 'Application Finalised', 'Last Action Code', 'Last Action Comment', 'Last Action Date');
	...
	for ($i = 0; $i < count($rows); $i++) {
		...
		*/
		...
		$objPHPExcel->getActiveSheet()->setCellValue('K'.$rowCount, $rows[$i]['Case_Initiation_Date']);
		
		$objPHPExcel->getActiveSheet()->setCellValue('L'.$rowCount, $rows[$i]['Proposed_Assignment_From_Date']);
		$objPHPExcel->getActiveSheet()->setCellValue('M'.$rowCount, $rows[$i]['Work_Ready_Estimated_Date']);
		$objPHPExcel->getActiveSheet()->setCellValue('N'.$rowCount, $rows[$i]['First_Contact_with_Applicant_Date']);
		$objPHPExcel->getActiveSheet()->setCellValue('O'.$rowCount, $rows[$i]['All_Documents_Received_Date']);
		$objPHPExcel->getActiveSheet()->setCellValue('P'.$rowCount, $rows[$i]['Application_sent_to_GE_Signature_Date']);
		$objPHPExcel->getActiveSheet()->setCellValue('Q'.$rowCount, $rows[$i]['All_Documentation_Received_for_Filing_Date']);
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$rowCount, $rows[$i]['Application_Filed_Date']);
		$objPHPExcel->getActiveSheet()->setCellValue('S'.$rowCount, $rows[$i]['Application_Finalised_Date']);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('V'.$rowCount, $rows[$i]['Last_Action_Date']);
